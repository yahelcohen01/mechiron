      ...
      {tab === 'select' && (
        <>
          {confirmSupplier ? (
            ...
            <p className="text-sm text-gray-500 py-4 text-center">אין ספקים זמינים בתחום זה</p>
          ) : (
            <div className="flex flex-col gap-1 max-h-60 overflow-y-auto">
              {availableSuppliers.map((supplier) => (
                <button
                  key={supplier.id}
                  type="button"
                  className="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-gray-100 text-start"
      ...
      {tab === 'create' && (
        <form onSubmit={handleCreateSubmit} className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1">
            <label className="text-sm font-medium text-gray-700">תחום</label>
